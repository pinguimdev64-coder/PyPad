...
def main():
    ...
    while True:
        ...
        for i in range(min(linha_livre + 1, area_texto)):
            cur(i, 0)
            prefixo = f"\033[41m\033[1;30m{i+1}|\033[0m "
        ...
        print(f"\033[41m\033[1;30mPyPad{'\033[0;31m█' * largura_blocos}\033[0m\033[41m\033[1;30m{linha}:{coluna}\033[0m\033[K", end="")
        ...
        if TECLA == key.ESC:
            ...
            # Se o cursor estiver no meio do texto, divide a linha

            KEYSNOR = [')', ']', '}']
            if coluna < len(linhas[linha]):
                if not linhas[linha][coluna] in KEYSNOR:
                    deslocar_BAIXO()
                    linha += 1
                    coluna = 0
                else:
                    try:
                        if linhas[linha][coluna-1] in ['[', '{', '(']:
                            linha_livre+=2
                            recorte = linhas[linha][coluna:]
                            linhas[linha] = linhas[linha][:coluna]
                            linhas[linha+2] = recorte
                            linha += 1
                            for a in range(4):
                                linhas[linha].append(" ")
                            coluna = len(linhas[linha])
                ...
                linha_livre += 1
                # Se estiver no final (ou linha vazia), só empurra as linhas de baixo
                for i in range(linha_livre, linha, -1):
                    linhas[i] = linhas[i - 1]
                linhas[linha + 1] = []
